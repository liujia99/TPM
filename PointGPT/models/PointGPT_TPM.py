...
class GPT_Transformer(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.num_masks = [int((self.num_groups - self.keep_attend) * mask_ratio) for mask_ratio in self.mask_ratios]
    ...
    def forward(self, neighborhood, center, noaug=False, classify=False):
        ...
        position = torch.cat([center[:, 0, :].unsqueeze(1), relative_position], dim=1)
        ...
        attn_mask = torch.full((seq_len, seq_len), -float("Inf"), device=group_input_tokens.device,
                               dtype=group_input_tokens.dtype).to(torch.bool)
        generated_points_list = []
        for num_mask in self.num_masks:
            with torch.no_grad():
                attn_mask = torch.triu(attn_mask, diagonal=1)
                overall_mask = np.hstack([
                    np.zeros(self.num_groups-self.keep_attend-num_mask),
                    np.ones(num_mask),
                ])
            ...
            # transformer
            encoded_features = self.blocks(group_input_tokens, pos_absolute, attn_mask, classify=classify)
            generated_points = self.generator_blocks(encoded_features, pos_relative, attn_mask)
            generated_points_list.append(generated_points)
        return generated_points_list

@MODELS.register_module()
class PointGPT_TPM(nn.Module):
    def __init__(self, config):
        ...
        self.mask_ratios = config.transformer_config.mask_ratios
        # self.GPT_Transformers = nn.ModuleList()
        # for mask_ratio in self.mask_ratios:
        #     self.GPT_Transformers.append(GPT_Transformer(config, mask_ratio))
    ...
    def forward(self, pts, vis=False, **kwargs):
        ...
        generated_points_list = self.GPT_Transformer(neighborhood, center)

        total_loss = 0
        for mask_id in range(len(self.mask_ratios)):
            gt_points = neighborhood.reshape(B * (self.num_group), self.group_size, 3)
            loss1 = self.loss_func_p1(generated_points_list[mask_id], gt_points)
            loss2 = self.loss_func_p2(generated_points_list[mask_id], gt_points)

            # if vis:  # visualization
            #     gt_points = gt_points.reshape(B, self.num_group, self.group_size, 3)
            #     gt_points = (gt_points + center.unsqueeze(-2)).reshape(-1, 3).unsqueeze(0)
            #     generated_points = generated_points.reshape(B, self.num_group, self.group_size, 3) + center.unsqueeze(-2)
            #     generated_points = generated_points.reshape(-1, 3).unsqueeze(0)
            #
            #     return generated_points, gt_points, center
            total_loss += (loss1 + loss2) * self.mask_ratios[mask_id] / sum(self.mask_ratios)
