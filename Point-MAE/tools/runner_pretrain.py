...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    list_best_accuracy = [0.0, 0.0, 0.0, 0.0]
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for mask_id in range(len(config.model.transformer_config.mask_ratios)):
            feats_train = []
            labels_train = []
            for i, (data, label) in enumerate(train_dataloader_svm):
                ...
                with torch.no_grad():
                    feats = base_model(data, mask_id, eval=True)
            ...
            print_log(f"MN40 Linear Accuracy : Mask{mask_id} - {test_accuracy}", logger=logger)
            if test_accuracy > list_best_accuracy[mask_id]:
                list_best_accuracy[mask_id] = test_accuracy
                print_log(f"Saving best...", logger=logger)
                builder.save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, f'ckpt-best-{mask_id}', args, logger=logger)
            if epoch % 25 ==0 and epoch >250:
                builder.save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, f'ckpt-epoch-{epoch:03d}-{mask_id}', args, logger=logger)
