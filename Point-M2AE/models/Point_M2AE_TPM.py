...
class H_Encoder(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.mask_ratios = config.mask_ratios
    ...
    def rand_mask(self, center, mask_ratio):
        B, G, _ = center.shape
    ...
    def forward(self, neighborhoods, centers, idxs, eval=False):
        ...
        x_vis_list_tpm, mask_vis_list_tpm, bool_masked_pos_tpm = [], [], []
        for mask_ratio in self.mask_ratios:
            ...
            if eval:
                ...
                bool_masked_pos.append(self.rand_mask(centers[-1], mask_ratio))
            ...
            x_vis_list_tpm.append(x_vis_list)
            mask_vis_list_tpm.append(mask_vis_list)
            bool_masked_pos_tpm.append(bool_masked_pos)

        return x_vis_list_tpm, mask_vis_list_tpm, bool_masked_pos_tpm


@MODELS.register_module()
class Point_M2AE_TPM(nn.Module):

    def __init__(self, config):
        super().__init__()
        print_log(f'[Point_M2AE_TPM]', logger ='Point_M2AE_TPM')
        self.config = config
        ...
        # for mask_ratio in self.mask_ratios:
        #     self.h_encoders.append(H_Encoder(config, mask_ratio))
    ...
    def forward(self, pts, mask_id = 0, eval=False, **kwargs):
        ...
        if eval:
            # for linear svm
            x_vis_list_tpm, mask_vis_list_tpm, _ = self.h_encoder(neighborhoods, centers, idxs, eval=True)
            x_vis = x_vis_list_tpm[mask_id][-1]
            return x_vis.mean(1) + x_vis.max(1)[0]
        else:
            x_vis_list_tpm, mask_vis_list_tpm, masks_tpm = self.h_encoder(neighborhoods, centers, idxs)
        ...
        for mask_id in range(len(self.mask_ratios)):
            x_vis_list_tpm[mask_id].reverse()
            masks_tpm[mask_id].reverse()

            for i in range(len(self.decoder_dims)):
                center = centers[i]
                # 1st-layer decoder, concatenate visible and masked tokens
                if i == 0:
                    x_full, mask = x_vis_list_tpm[mask_id][i], masks_tpm[mask_id][i]
                    B, _, C = x_full.shape
                    ...
                    x_vis = x_vis_list_tpm[mask_id][i]
                    bool_vis_pos = ~masks_tpm[mask_id][i]
                    mask_vis = mask_vis_list_tpm[mask_id][i]
                    B, N, _ = center.shape
                    _, _, C = x_vis.shape
            ...
            B, N, C = x_full.shape
            x_rec = x_full[masks_tpm[mask_id][-2]].reshape(-1, C)
            L, _ = x_rec.shape

            rec_points = self.rec_head(x_rec.unsqueeze(-1)).reshape(L, -1, 3)
            gt_points = neighborhoods[-2][masks_tpm[mask_id][-2]].reshape(L, -1, 3)
