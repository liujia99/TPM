...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        # print('x_vis=>', x_vis.shape, 'mask=>', mask.shape, mask)
        ...
        # print('pos_emd_vis=>', pos_emd_vis.shape) pos_emd_vis=> torch.Size([32, 26, 384])
        pos_emd_mask = self.decoder_pos_embed(center[mask]).reshape(B, -1, C)
        # print('pos_emd_mask=>', pos_emd_mask.shape) pos_emd_mask=> torch.Size([32, 38, 384])

        _,N,_ = pos_emd_mask.shape
        mask_token = self.mask_token.expand(B, N, -1)
        x_full = torch.cat([x_vis, mask_token], dim=1)
        pos_full = torch.cat([pos_emd_vis, pos_emd_mask], dim=1)
        # print('x_full=>', x_full.shape, 'pos_full=>', pos_full.shape)
        # x_full = > torch.Size([32, 64, 384])
        # pos_full = > torch.Size([32, 64, 384])
...
class PointTransformer(nn.Module):
    ...
    def load_model_from_ckpt(self, bert_ckpt_path):
        if bert_ckpt_path is not None:
            ...
            #print('base_ckpt.keys(0)=>', base_ckpt.keys())
            #base_ckpt = {k.replace("MAE_encoders.2.", "MAE_encoder."): v for k, v in base_ckpt.items()}
            #print('base_ckpt.keys(1)=>', base_ckpt.keys())
